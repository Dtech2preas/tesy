...
<body>
    ...
    <script>
        ...
        function calculateLikelihood(course) {{
            ...
            if (!meetsSubjects) {{
                return {{ percent: 0, reason: `Failed subject requirements: ${{subjectFails.join(', ')}}` }};
            }}

            if (targetScore > 0 && userScore < targetScore) {{
                let scoreType = isFps ? "FPS" : "APS";
                return {{ percent: 0, reason: `Your ${{scoreType}} (${{userScore}}) is below required (${{targetScore}})` }};
            }}

            if (targetScore === 0) {{
                // Met subject requirements, but no score specified
                return {{ percent: 75, reason: "Met subject requirements. No specific APS/FPS listed." }};
            }}

            // Calculation formula
            let pointsAbove = userScore - targetScore;
            let baseLikelihood = 50;

            // Add 5% per APS point, or 1% per 2 FPS points
            let additionalLikelihood = isFps ? Math.floor(pointsAbove / 2) : (pointsAbove * 5);

            let totalLikelihood = baseLikelihood + additionalLikelihood;
            if (totalLikelihood > 100) totalLikelihood = 100;

            let scoreType = isFps ? "FPS" : "APS";
            return {{ percent: totalLikelihood, reason: `Met all requirements! Your ${{scoreType}} is ${{pointsAbove}} points above the minimum (${{targetScore}}).` }};
        }}
